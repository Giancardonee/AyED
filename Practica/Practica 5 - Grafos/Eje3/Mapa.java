...
public class Mapa {
    ...
    private boolean[] marcarRestringidos(List<String> lugaresRestringidos) {
        boolean[] marca = new boolean[this.mapaCiudades.getSize()];
        for (String actual : lugaresRestringidos) {
            Vertex<String> verticeActual = this.mapaCiudades.search(actual);
    ...
    public List<String> devolverCaminoExceptuando(String ciudad1, String ciudad2, List<String> ciudadesRestringidas) {
        ...
        if (mapaCiudades != null && !mapaCiudades.isEmpty()) {
            ...
            if (origen != null && destino != null) {
                boolean[] marca = marcarRestringidos(ciudadesRestringidas);
                caminoExceptuandoHelper(origen, destino, listaReturn, ciudadesRestringidas, marca);
            }
        }
        return listaReturn;
    }
            ...
            for (Edge<String> ady : adyacentes) {
                ...
                if (!marca[j]) {
                    if (tanqueAuto - costoCombustible <= 0) {
                        tanqueAuto = 100; // cambiar valores para ir testeando
                        vecesCargadas++;
                    }
    ...
    /*
    Esto es ineficiente por el uso del .contains
