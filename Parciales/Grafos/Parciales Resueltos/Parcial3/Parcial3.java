...
public class Parcial3 {
    ...
    private Vertex<Recinto> buscarVerticeEntrada(Graph<Recinto> sitios) {
        Vertex<Recinto> vReturn = null;
        boolean encontre = false;
        List<Vertex<Recinto>> vertices = sitios.getVertices();
        Iterator<Vertex<Recinto>> iterador = vertices.iterator();
        while (iterador.hasNext() && !encontre) {
            vReturn = iterador.next();
            if (vReturn.getData().getNombre().equals("Entrada")) {
                encontre = true;
            }
        }
        return vReturn;
    ...
    // con recorrido en profundidad: DFS 
    ...
    public static void main(String args[]) {
        ...
        System.out.println(p.resolver(grafo, 204));
