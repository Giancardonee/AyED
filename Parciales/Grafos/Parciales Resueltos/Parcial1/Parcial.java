...
public class Parcial {
    ...
    private Vertex<Ciudad> buscarCiudadEnMapa (String ciudad, Graph<Ciudad> mapa)
    {
        ...
        List<Vertex<Ciudad>> adyacentes = mapa.getVertices();
        Iterator<Vertex<Ciudad>> iterador = adyacentes.iterator();
        boolean encontre=false;
        while (iterador.hasNext() && !encontre)
        {
            origen = iterador.next();
            if (origen.getData().getNombre().equals(ciudad))
            {
                encontre=true;
            }
        }
